...
WHERE PINC_ID = " & Val(ViewState("p_no")), insurance_SQLcon)
    ...
    Private Sub getProcessesData()
        ...
        Try
            Dim sql_str As String = "SELECT INC_CompanyProcesses.Processes_ID, Processes_Reservation_Code, INC_CompanyProcesses.PINC_ID, convert(varchar, Processes_Date, 23) AS Processes_Date, Processes_Time, Clinic_AR_Name AS Processes_Cilinc, SubService_AR_Name AS Processes_SubServices, ISNULL(INC_MOTALBA_PRICES.Processes_Price, INC_CompanyProcesses.Processes_Price) AS Processes_Price, ISNULL(INC_MOTALBA_PRICES.Processes_Paid, INC_CompanyProcesses.Processes_Paid) AS Processes_Paid, INC_CompanyProcesses.Processes_Residual, ISNULL(MedicalStaff_AR_Name, '') AS MedicalStaff_AR_Name, ISNULL(INC_CompanyProcesses.NAME_ARB, '') AS PATIENT_NAME FROM INC_CompanyProcesses
