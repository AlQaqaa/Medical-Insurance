...
#Region "Varaibles & Properties"
    ...
    ''' Double Part
    ''' </summary>
    Private _DoubleValue As Integer
...
#Region "General"
    ...
    Public Sub New(ByVal number As [Double], ByVal currency As CurrencyInfo)
    ...
    Public Sub New(ByVal number As [Double], ByVal currency As CurrencyInfo, ByVal englishPrefixText As [String], ByVal englishSuffixText As [String], ByVal arabicPrefixText As [String], ByVal arabicSuffixText As [String])
    ...
    ''' <param name="arabicPrefixText">Arabic text to be placed before the generated text</param>
    ''' <param name="arabicSuffixText">Arabic text to be placed after the generated text</param>
    Private Sub InitializeClass(ByVal number__1 As [Double], ByVal currency__2 As CurrencyInfo, ByVal englishPrefixText__3 As [String], ByVal englishSuffixText__4 As [String], ByVal arabicPrefixText__5 As [String], ByVal arabicSuffixText__6 As [String])
        Number = number__1
    ...
    ''' </summary>
    ''' <param name="DoublePart">Double Part as a String</param>
    ''' <returns></returns>
    Private Function GetDoubleValue(ByVal DoublePart As String) As String
        Dim result As String = [String].Empty

        If Currency.PartPrecision <> DoublePart.Length Then

            Dim DoublePartLength As Integer = DoublePart.Length

            For i = 0 To Currency.PartPrecision - DoublePartLength
                DoublePart += "0" 'Fix for 1 number after Double ( 10.5 , 1442.2 , 375.4 ) 
            Next
...
#End Region
    ...
    Public Function ConvertToEnglish() As String
        Dim tempNumber As [Double] = Number

        If tempNumber = 0 Then
            Return "Zero"
        End If

        Dim DoubleString As String = ProcessGroup(_DoubleValue)
        ...
        formattedNumber += If((DoubleString <> [String].Empty), " and ", [String].Empty)
        formattedNumber += If((DoubleString <> [String].Empty), DoubleString, [String].Empty)
        formattedNumber += If((DoubleString <> [String].Empty), " " & Convert.ToString((If(_DoubleValue = 1, Currency.EnglishCurrencyPartName, Currency.EnglishPluralCurrencyPartName))), [String].Empty)
...
#End Region
    ...
    ''' <param name="groupNumber">The group number to process</param>
    ''' <returns></returns>
    Private Function ProcessArabicGroup(ByVal groupNumber As Integer, ByVal groupLevel As Integer, ByVal remainingNumber As [Double]) As String
        Dim tens As Integer = groupNumber Mod 100
    ...
    Public Function ConvertToArabic() As String
        ...
        Dim DoubleString As String = ProcessArabicGroup(_DoubleValue, -1, 0)
        ...
        If _intergerValue <> 0 Then
            ...
            End If
        End If
        formattedNumber += If((_DoubleValue <> 0), " و ", [String].Empty)
        formattedNumber += If((_DoubleValue <> 0), DoubleString, [String].Empty)
        If _DoubleValue <> 0 Then
            ' here we add currency part name depending on _intergerValue : 1 ,2 , 3--->10 , 11--->99
            formattedNumber += " "

            Dim remaining100 As Integer = CInt(_DoubleValue Mod 100)
