...
Public Class companyStatistics
    ...
    Sub getMoneyInfo()
        Try
            Dim sel_com As New SqlCommand("SELECT SUM(Processes_Residual) AS C_COUNT FROM INC_CompanyProcesses WHERE Processes_State <> 3 AND C_ID = " & Session("company_id") & " AND Processes_Date >= '" & ViewState("start_dt") & "'", insurance_SQLcon)
    ...
    Sub bindChartsSubServices()
        Try
            Dim sql_str As String = "SELECT TOP (10) (SELECT SubService_AR_Name FROM Main_SubServices WHERE Main_SubServices.SubService_ID = INC_CompanyProcesses.Processes_SubServices) AS SubService_AR_Name, COUNT(*) AS SubService_COUNT FROM INC_CompanyProcesses WHERE C_ID = " & Session("company_id") & " AND Processes_Date >= '" & ViewState("start_dt") & "' AND Processes_State <> 3 GROUP BY Processes_SubServices ORDER BY COUNT(*) DESC"
    ...
    Sub bindChartsDoctros()
        Try
            Dim sql_str As String = "SELECT TOP (10) (SELECT MedicalStaff_AR_Name FROM Main_MedicalStaff WHERE Main_MedicalStaff.MedicalStaff_ID = INC_CompanyProcesses.doctor_id) AS MedicalStaff_AR_Name, COUNT(*) AS doctors_COUNT FROM INC_CompanyProcesses WHERE C_ID = " & Session("company_id") & " AND Processes_Date >= '" & ViewState("start_dt") & "' AND doctor_id <> 0 AND Processes_State <> 3 GROUP BY doctor_id ORDER BY COUNT(*) DESC"
    ...
    Sub getSummary()
        Dim sql_str As String = "SELECT ISNULL(AVG(Processes_Residual),0) AS AVG_EXPENS, ISNULL(SUM(Processes_Residual),0) AS TOTAL_EXPENS, ISNULL(COUNT(*),0) AS TOTAL_SERVICES FROM INC_CompanyProcesses WHERE C_ID = " & Session("company_id") & " AND Processes_Date >= '" & ViewState("start_dt") & "' AND Processes_State <> 3"
    ...
    Sub getPetientCount()
        Dim sql_str As String = "SELECT COUNT(DISTINCT Processes_Reservation_Code) AS p_count FROM INC_CompanyProcesses WHERE C_ID = " & Session("company_id") & " AND Processes_Date >= '" & ViewState("start_dt") & "' AND Processes_State <> 3"
